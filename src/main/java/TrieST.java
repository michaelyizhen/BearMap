...
public class TrieST<Value> {
   ...
   /**
    ...
    public Iterable<Value> keysWithPrefix(String prefix) {
        Deque<Value> results = new LinkedList<>();
    ...
    private void collect(Node x, StringBuilder prefix, Queue<Value> results) {
        if (x == null) return;
        if (x.val != null) results.offer(get(prefix.toString()));
        for (char c = 0; c < R; c++) {
            prefix.append(c);
     ...
     */
    public Iterable<Value> keysThatMatch(String pattern) {
        Deque<Value> results = new LinkedList<>();
        collect(root, new StringBuilder(), pattern, results);
        return results;
    }

    private void collect(Node x, StringBuilder prefix, String pattern, Queue<Value> results) {
        if (x == null) return;
        int d = prefix.length();
        if (d == pattern.length() && x.val != null)
            results.offer(get(prefix.toString()));
        if (d == pattern.length())
            return;
